...
try {
  ...
  console.log('⚠️ [Auto-Optimize] Module sharp non disponible. Saut de la compression d\'images.');
  process.exit(0);
}

// 1. Détection dynamique de la racine du projet Expo (en cherchant app.config.js / app.json)
function findProjectRoot(startDir) {
  ...
  // Fallback si pas trouvé en remontant : chercher dans les sous-dossiers immédiats
  try {
...
const IMAGE_SIZE_THRESHOLD = 150 * 1024; // Seuil de compression : 150 KB
...
async function runOptimization() {
  ...
  for (const fileObj of allFiles) {
    ...
    // 1. Compression dynamique des images (.png, .jpg, .jpeg, .webp)
    if (['.jpg', '.jpeg', '.png', '.webp'].includes(ext) && size > IMAGE_SIZE_THRESHOLD) {
      ...
      try {
        ...
        if (fs.existsSync(tempPath)) fs.unlinkSync(tempPath);
        console.warn(`  ⚠️ Non modifiée ${relativePath}: ${err.message}`);
      }
    }
    // 2. Détection et alerte sur les fichiers vidéo ou documents lourds hors code
    else if (HEAVY_FILE_EXTENSIONS.has(ext) || (size > 500 * 1024 && !['.json', '.ts', '.tsx', '.js', '.map'].includes(ext))) {
      if (size > 500 * 1024) {
        heavyWarningsCount++;
        console.warn(`  ⚠️ [Attention Fichier Lourd] ${relativePath} (${(size / 1024 / 1024).toFixed(2)} MB)`);
      }
    }
  }
  ...
  if (heavyWarningsCount > 0) {
    console.warn(`  ⚠️ ${heavyWarningsCount} fichier(s) vidéo/media lourd(s) détecté(s). Pensez à les héberger à distance.`);
  }
}

runOptimization().catch((err) => {
  console.error('⚠️ [Auto-Optimize] Erreur lors de l\'exécution:', err);
});
