...
export type NumberRiskStatus = 'securise' | 'a_signaler' | 'frauduleux' | 'unknown' | string;
...
export default function NumberStatusBadge({ status }: NumberStatusBadgeProps) {
  ...
  const normalized = (status || '').toLowerCase().replace(/é/g, 'e').replace(/à/g, 'a').replace(/\s+/g, '_');
  const key: 'securise' | 'a_signaler' | 'frauduleux' | 'unknown' =
    normalized.includes('secu') ? 'securise' :
    normalized.includes('sign') ? 'a_signaler' :
    normalized.includes('fraud') ? 'frauduleux' : 'unknown';

  const configs = {
    securise: {
      icon: 'solar:shield-check-bold',
      style: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30',
      labelKey: 'numbers.statusSecured',
    },
    a_signaler: {
      icon: 'solar:danger-triangle-bold',
      style: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30',
      labelKey: 'numbers.statusWarning',
    },
    frauduleux: {
      icon: 'solar:user-block-bold',
      style: 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/30',
      labelKey: 'numbers.statusFraud',
    },
    unknown: {
